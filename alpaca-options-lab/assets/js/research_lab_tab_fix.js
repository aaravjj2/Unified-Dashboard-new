...
(function() {
    ...
    if (window.location.port !== '8058') {
        console.debug('[Research Lab Fix] Skipping - not on Research Lab');
        return;
    }
    
    console.debug('[Research Lab Fix] Injecting tab labels...');
    ...
    function injectLabels() {
        ...
        if (injected > 0) {
                console.debug(`[Research Lab Fix] ✓ Injected ${injected} tab labels`);
            ...
            if (tabs.length >= 4) {
                ...
                if (panes.length >= 4) {
                    ...
                    console.debug('[Research Lab Fix] ✓ Activated Scenario Lab tab by default');
                }
            }
