    ...
    def handle_chat_message(send_clicks, message, current_messages, session_id, active_tab):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Chatbot service unavailable on port 8062, trying fallbacks: {conn_err}")
                service_available = False
            
            # Try internal RAG orchestrator as fallback
            if not service_available:
                try:
                    rag = get_rag()
                    if rag is not None:
                        # Use internal RAG to generate response
                        result = rag.answer(
                            query=message,
                            session_id=session_id or "default",
                            tab_context=active_tab or "dashboard"
                        )
                        answer = result.get('answer', 'I processed your request.')
                        ...
                        # If there's an action suggestion, store it
                        pending_action = action if action else None
                        
                        logger.info(f"✅ Internal RAG response: {len(answer)} chars")
                        return current_messages, "", pending_action
                except Exception as rag_err:
                    logger.warning(f"Internal RAG failed: {rag_err}")
                
                # Try Gemini/OpenAI fallback
                try:
                    from financial_dashboard.services.chat.ai_fallback import get_fallback_chat
                    fallback = get_fallback_chat()
                    if fallback.is_available():
                        result = fallback.chat(
                            message=message,
                            context=f"Current tab: {active_tab or 'dashboard'}"
                        )
                        ...
                        logger.info(f"✅ AI Fallback ({model}) response: {len(answer)} chars")
                        return current_messages, "", None
                except Exception as fallback_err:
                    logger.warning(f"AI fallback failed: {fallback_err}")
                
                # Ultimate fallback - helpful message
                fallback_message = """I'm your AI Financial Assistant. While the advanced AI model is loading, I can still help you navigate the dashboard.

**Quick Actions:**
• Type "show volatility" to open Volatility Lab
• Type "show portfolio" to view your positions
• Type "market trends" to see market data

**Features Available:**
• 📊 Market data and real-time charts
• 💹 Options analysis with IV Surface
...
What would you like to explore?"""
    ...
    def update_chatbot_status(n):
        ...
        active_style = {
            "fontSize": "12px",
            "marginLeft": "10px",
            "color": "#4ade80",  # Green - Active is good!
            "backgroundColor": "rgba(0,0,0,0.2)",
            "padding": "2px 8px",
            "borderRadius": "10px",
            "transition": "all 0.3s ease"
        ...
        # Try external chatbot service first (GPU-accelerated or smart rule-based)
        try:
            ...
            if response.status_code == 200:
                ...
                if llm_available:
                    return "● Online (GPU)", online_style
                else:
                    return "● Ready", active_style  # Show green "Ready" for rule-based mode
        except Exception:
            pass
        
        # Check if internal RAG is available
        try:
            rag = get_rag()
            if rag is not None:
                return "● Ready", active_style  # Show green "Ready" instead of yellow "Fallback"
        except Exception:
            pass
            
        return "● Offline", offline_style
