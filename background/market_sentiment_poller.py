...
def _poll_sentiment_sources() -> Dict[str, Any]:
    ...
    # Helper to load source module robustly (supports alternate package roots)
    def _load_source_callable(module_name: str, func_name: str = "get_market_sentiment"):
        candidates = [module_name, f"financial_dashboard.{module_name}"]
        for m in candidates:
            try:
                ...
                logger.exception(f"Error importing {m}")
                continue
        raise ModuleNotFoundError(module_name)

    # Poll Finnhub (Priority 1)
    try:
        finnhub_sentiment = _load_source_callable("services.cc.ingest_finnhub")
        finnhub_result = finnhub_sentiment()

        if finnhub_result.get("error") is None:
            source_scores["finnhub"] = finnhub_result.get("score", 0.0)
            active_sources.append("finnhub")
            logger.debug(f"Finnhub sentiment: {source_scores['finnhub']:.3f}")
        else:
            logger.warning(f"Finnhub error: {finnhub_result.get('error')}")
            source_scores["finnhub"] = None
    except ModuleNotFoundError:
        logger.error("Failed to fetch Finnhub sentiment - module not found")
        source_scores["finnhub"] = None
    except Exception:
    ...
    try:
        alpaca_sentiment = _load_source_callable("services.cc.alpaca_market")
        alpaca_result = alpaca_sentiment()

        if alpaca_result.get("error") is None:
            source_scores["alpaca"] = alpaca_result.get("score", 0.0)
            active_sources.append("alpaca")
            logger.debug(f"Alpaca sentiment: {source_scores['alpaca']:.3f}")
        else:
            logger.debug(f"Alpaca skipped: {alpaca_result.get('error')}")
            source_scores["alpaca"] = None
    except ModuleNotFoundError:
        logger.error("Failed to fetch Alpaca sentiment - module not found")
        source_scores["alpaca"] = None
    except Exception:
    ...
    try:
        yfinance_sentiment = _load_source_callable("services.cc.yfinance_fallback")
        yfinance_result = yfinance_sentiment()

        if yfinance_result.get("error") is None:
            source_scores["yfinance"] = yfinance_result.get("score", 0.0)
            active_sources.append("yfinance")
            logger.debug(f"yfinance sentiment: {source_scores['yfinance']:.3f}")
        else:
            logger.debug(f"yfinance skipped: {yfinance_result.get('error')}")
            source_scores["yfinance"] = None
    except ModuleNotFoundError:
        logger.error("Failed to fetch yfinance sentiment - module not found")
        source_scores["yfinance"] = None
    except Exception:
