...
TAB_CONFIG = [
    {'id': 'home_lab', 'name': '🏠 Command Center', 'module': 'tabs/home_lab/__init__.py'},
    {'id': 'home', 'name': '🏠 Home', 'module': 'tabs/home.py'},  # Legacy home (keep for compatibility)
    ...
    # PHASE 4 - AGENT 1B: Azure ML Lab - Machine learning predictions and analytics
    {'id': 'azure_ml_lab', 'name': '🤖 Azure ML Lab', 'module': 'tabs/azure_ml_lab/__init__.py'},
    # TEMPORARILY DISABLED: Backtesting Lab has duplicate callback errors due to @callback decorators at module level
    # {'id': 'backtesting_lab', 'name': '📊 Backtesting Lab', 'module': 'tabs/backtesting_lab.py'},
]

# Module-level constant for enabled tabs (used by create_layout)
ENABLED_TABS = [
    'home_lab',          # COMMAND CENTER: Primary dashboard
    ...
    'azure_ml_lab',      # PHASE 4: Azure ML predictions (Agent 1B)
    'weekly_picks',
    'monthly_picks',
    'market_trends',
    'market_forecast',
...
for tab_config in TAB_CONFIG:
    try:
        ...
        if tab_config['id'] in ('options_lab', 'attribution_lab', 'strategy_lab', 'research_lab', 'home_lab', 'azure_ml_lab'):
...
def create_layout():
        ...
        if tab_key in loaded_tabs:
            try:
                ...
                if hasattr(tab_info['module'], 'layout'):
                    layout_func = tab_info['module'].layout
                    content = layout_func() if callable(layout_func) else layout_func
                    logger.info(f"  ✅ Created layout for {tab_key}")
                else:
    ...
    layout = dbc.Container([
        ...
        # These are generated at layout build time from persisted outputs so
        # Playwright can reliably read authoritative prices even if background
        # fetches haven't completed yet in the running process.
        (lambda: (
            (lambda w, m: [
                ...
                (lambda: (open(os.path.join(APP_DIR, 'outputs', 'prices_weekly.json'), 'r', encoding='utf-8').read()) if os.path.exists(os.path.join(APP_DIR, 'outputs', 'prices_weekly.json')) else '{}')(),
                (lambda: (open(os.path.join(APP_DIR, 'outputs', 'prices_monthly.json'), 'r', encoding='utf-8').read()) if os.path.exists(os.path.join(APP_DIR, 'outputs', 'prices_monthly.json')) else '{}')()
        ))(),
        ...
        html.Div([
            ...
            dcc.Tabs(id='tabs', value='tab-1', children=[])  # Hidden for legacy
        ], style={'display': 'none'}),
...
if __name__ == '__main__':
    ...
    # Import app from app.py (module-level singleton)
    from app import app
    
    # Get port from environment variable (default 8050 to avoid conflicts)
