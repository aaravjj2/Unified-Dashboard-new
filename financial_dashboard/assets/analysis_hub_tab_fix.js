...
// GUARD: Only run on Analysis Hub (port 8054) or Unified Dashboard (8055)
if (window.location.port !== '8054' && window.location.port !== '8055' && window.location.port !== '') {
    console.log('[Analysis Hub] Skipping - not on Analysis Hub/Unified Dashboard');
    // Exit immediately - no modifications
} else {
...
function activateFirstTab() {
    try {
        ...
        if (tabPanes.length > 0) {
            console.log(`[Analysis Hub] Found ${tabPanes.length} tab panes`);
            ...
            if (!hasActive) {
                ...
                if (firstTabLink) {
                    ...
                    console.log('[Analysis Hub] Activated first tab link');
                }
...
function fixTabLabels() {
    try {
        ...
        const expectedLabels = ['Attribution Analysis', 'Portfolio Analytics', 'Scenario Testing'];
        
        let fixed = 0;
        tabs.forEach((tab, index) => {
            // Check if tab is empty or only has whitespace
...
const retryInterval = setInterval(function() {
    ...
    if (activateFirstTab() || retries >= maxRetries) {
        ...
        console.log(`[Analysis Hub] Tab activation ${retries <= maxRetries && activateFirstTab() ? 'succeeded' : 'gave up'} after ${retries} attempts`);
    }
}, 500);

// Close the guard clause
}
