...
(function() {
    ...
    if (window.location.port !== '8056' && window.location.port !== '8055' && window.location.port !== '') {
        console.debug('[Portfolio Fix] Skipping - not on Portfolio/Unified Dashboard');
        return;
    }
    ...
    function injectTabLabels() {
        ...
        if (tabs.length === 0) {
            console.debug('[Tab Fix] Tabs not found yet, retrying...');
            return false;
        }
        
        console.debug(`[Tab Fix] Found ${tabs.length} tabs, injecting labels...`);
        ...
        tabs.forEach((tab, index) => {
            if (index < tabLabels.length) {
                ...
                if (!tab.textContent || tab.textContent.trim().length < 3) {
                    ...
                    console.debug(`[Tab Fix] Injected label "${tabLabels[index]}" into tab ${index}`);
                }
            }
        });
        return true;
    ...
    console.debug('[Tab Fix] Portfolio tab label injector loaded');
})();
