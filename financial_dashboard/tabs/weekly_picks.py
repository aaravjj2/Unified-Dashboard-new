...
def _load_and_enrich_picks():
    ...
    try:
        ...
        try:
            ...
            logger.debug('DB read failed or DB unavailable; falling back to JSON/CSV')
        ...
        try:
            ...
            if os.path.exists(picks_dir):
                # Find latest weekly picks JSON
                json_files = sorted([f for f in os.listdir(picks_dir) if f.startswith('weekly_picks') and f.endswith('.json')], reverse=True)
                if json_files:
                    json_path = os.path.join(picks_dir, json_files[0])
