"""
Monthly Picks Package - UI Components with Stable IDs

Implements dry-run/approve workflow for monthly picks with:
- Stable element IDs (mp-*)
- Diff visualization
- Admin approve modal
...
def create_layout(initial_data=None):
    """
    Create Monthly Picks layout with stable IDs.
    ...
    return html.Div([
        # Header
        html.Div([
            html.H3('Monthly Picks', style={'color': 'white'}),
            html.Span(
                'Pipeline Status: Ready',
                id='mp-pipeline-status',
                style={
        ...
        html.Div([
            html.H4('Published Monthly Picks', style={'color': 'white', 'marginBottom': '8px'}),
            html.Div(
                'No picks published yet. Run pipeline to generate.',
                id='mp-published-table',
        ...
        # Hidden stores
        dcc.Store(id='mp-current-run-id', data=None),
        dcc.Store(id='mp-staging-data', data=None),
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                ...
                html.P('This will replace the currently published monthly picks.', style={'color': '#ef4444'}),
                html.Div([
                    html.Label('Admin Token:', style={'marginBottom': '4px'}),
                    dcc.Input(
                        id='mp-admin-token-input',
                        type='password',
            ...
            dbc.ModalFooter([
                dbc.Button('Cancel', id='mp-approve-cancel', color='secondary'),
                dbc.Button('Confirm Publish', id='mp-approve-confirm', color='success')
            ])
        ], id='mp-approve-modal', is_open=False)
