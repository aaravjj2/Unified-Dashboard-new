...
def layout():
    ...
    return html.Div([
        ...
        html.Button('Backtest Trend Signals', id='backtest-btn', n_clicks=0, style={'marginLeft': '8px', 'backgroundColor': '#10b981', 'color': 'white'}),
        html.Button('🔍 Debug Logs', id='debug-logs-btn', n_clicks=0, style={'marginLeft': '8px', 'backgroundColor': '#f59e0b', 'color': 'white', 'fontSize': '12px'}),
...
def register_callbacks(app):
    ...
    logger.info("✅ Market Trends: Cache Manager and News Manager initialized")
    logger.info(f"✅ Market Trends: Fixed callbacks registered")
    ...
    @app.callback(
        Output('results-area', 'children', allow_duplicate=True),  # FIX: Added allow_duplicate=True
        ...
        Output('trends-last-cached', 'data'),  # PHASE 3: Track last render timestamp
    ...
    def render_on_tab_activation(active_tab, job_id, last_cached_timestamp):
        ...
        try:
            ...
            if last and (last.get('detailed') or last.get('tidy')):
                ...
                if data:
                    ...
                    return composite, indicator_msg, indicator_style, news_elements, cache_timestamp
        ...
        return empty_msg, indicator_msg, indicator_style, no_news, None
        # MISSION A3: No data means no news
        no_news = html.Div(
            'No news available (no tickers loaded)',
            **{'data-testid': 'news-panel', 'style': {'padding': '16px', 'color': '#94a3b8', 'textAlign': 'center'}}
    ...
    @app.callback(
        ...
        prevent_initial_call=True  # This callback only runs when user clicks "Run Analysis"
    )
    def update_results_and_poll(n_clicks, n_intervals, queued_job_id, reload_data, tickers, period, job_id, analysis_options):
        ...
        logger.info("🔄 Analysis callback invoked: triggered_id=%s, n_clicks=%s, ctx.triggered=%s", triggered_id, n_clicks, ctx.triggered)
        ...
        if triggered_id == 'poll-interval' and job_id:
            try:
                ...
                try:
                    ...
                    _logger.info(f"Polling for job_id={job_id}. SH.JOBS keys: {list(jobs_dict.keys())}")
