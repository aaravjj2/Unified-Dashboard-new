...
def _find_latest_monthly_picks():
    """Find the most recent monthly picks CSV or JSON."""
    ...
    # PHASE 26: Check for JSON picks files in data/picks directory first
    data_picks_dir = os.path.join(dash_root, 'data', 'picks')
    if os.path.isdir(data_picks_dir):
        json_files = sorted(glob.glob(os.path.join(data_picks_dir, 'monthly_picks*.json')), reverse=True)
        if json_files:
            selected = json_files[0]  # Most recent by name
            logger.info(f"Selected monthly picks JSON from data/picks: {selected}")
            return selected
...
def _load_and_enrich_picks():
    ...
    try:
        ...
        summary = {
            ...
            'winners': winners,
            'losers': losers,
            'csv_path': data_path,  # PHASE 26: Fixed from csv_path to data_path
            'update_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
...
def layout():
    ...
    return html.Div([
        ...
        html.Div([
            create_tab_toolbar(
                tab_name="monthly_picks",
                filters=[create_sector_filter('picks-sector')] if SHARED_UI_AVAILABLE else [],
                show_refresh=True,
                show_export=True,
                show_help=True,
                help_text="Monthly stock picks based on quantitative analysis."
        ...
        html.Div([
            create_summary_stats_row([
                {'title': 'Total Picks', 'value': '--', 'icon': 'fa-list', 'color': 'primary'},
            {'title': 'Avg Return', 'value': '--%', 'icon': 'fa-chart-line', 'color': 'success'},
            {'title': 'Win Rate', 'value': '--%', 'icon': 'fa-trophy', 'color': 'warning'},
            {'title': 'Updated', 'value': '--', 'icon': 'fa-calendar', 'color': 'info'}
            ]) if SHARED_UI_AVAILABLE else html.Div()
        ]),
        
        # === IMPROVEMENTS: Notification Toast ===
        html.Div([
            create_notification_toast("monthly_picks-toast", "Monthly Picks Update") if SHARED_UI_AVAILABLE else html.Div()
        ]),
