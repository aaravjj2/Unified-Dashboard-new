...
def register_callbacks(app, SH=None):
    ...
    def _poll(n_intervals, current_job):
        ...
        # Fallback: query API gateway for job status/result if local registries don't have it
        if job is None:
            try:
                import requests, os
                gw = os.environ.get('API_GATEWAY_URL', 'http://127.0.0.1:8049')
                resp = requests.get(f"{gw}/api/trends/jobs/{current_job}", timeout=3)
                if resp.status_code == 200:
                    job = resp.json()
                    # Normalize to local job dict shape
                    job = {'status': job.get('status'), 'result': job.get('result')}
            except Exception:
                job = None
        if not job:
