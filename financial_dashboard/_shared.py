        ...
        try:
            ...
            try:
                with time_limit(job_timeout):
                    ...
                    # WORKAROUND: Write result to temp file so polling can find it even if JOBS dict is different instance
                    try:
                        import tempfile
                        result_file = os.path.join(tempfile.gettempdir(), f"{jid}_result.json")
                        with open(result_file, 'w') as f:
                            json.dump({'status': 'done', 'result': res}, f)
                        _logger.critical(f"💾 Job result written to {result_file} for cross-instance polling")
                    except Exception as write_err:
                        _logger.error(f"Failed to write job result file: {write_err}")
