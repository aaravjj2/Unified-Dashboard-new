...
# Lightweight job-status endpoint for compatibility with tests
try:
    if server is not None:
        @server.route('/api/jobs/<job_id>', methods=['GET'])
        def _api_get_job_status(job_id):
            try:
                # Prefer in-process shared job registry
                if 'SH' in globals() and SH is not None and hasattr(SH, 'get_job_status'):
                    status = SH.get_job_status(job_id)
                    if not status:
                        return jsonify({'error': f'Job {job_id} not found'}), 404
                    return jsonify(status)

                # Fallback: attempt to read tmp result file written by background jobs
    ...
    # Non-fatal: if Flask server isn't available, continue without the route
    pass
