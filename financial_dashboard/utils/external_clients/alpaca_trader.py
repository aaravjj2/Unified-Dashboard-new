...
class AlpacaTrader:
    ...
    def place_option_order(
        ...
        """
        Place an options contract order to Alpaca.
        
        Args:
            symbol: Underlying stock symbol (e.g., "SPY", "AAPL")
            option_type: "call" or "put"
            expiration: Expiration date in YYYY-MM-DD format (e.g., "2025-12-20")
            strike: Strike price (e.g., 450.0)
            qty: Number of contracts (not shares - each contract = 100 shares)
            side: "buy_to_open", "sell_to_close", "sell_to_open", "buy_to_close"
            order_type: "market" or "limit"
            limit_price: Limit price per contract (required for limit orders)
            time_in_force: "day" or "gtc"
            
        Returns:
            Dictionary with 'success', 'order_id', 'error', etc.
            
        Example:
            >>> # Buy 1 SPY call contract
            ...
            ... )
        """
        # Build OCC symbol (Alpaca's format for options)
        # Format: {TICKER}{YYMMDD}{C/P}{STRIKE*1000:08d}
        # Example: SPY251220C00450000 = SPY Call expiring 2025-12-20 strike $450
        try:
            ...
            occ_symbol = f"{symbol.upper()}{yymmdd}{cp}{strike_int:08d}"
            
            logger.info(f"📊 Options order: {occ_symbol} ({option_type} {strike} @ {expiration})")
        except Exception as e:
            logger.error(f"❌ Invalid option parameters: {e}")
            return {"success": False, "error": "invalid_parameters", "detail": str(e)}
        
        # Validate
        if qty <= 0:
            return {"success": False, "error": "invalid_quantity", "detail": "Quantity must be positive"}
        
        if order_type == "limit" and limit_price is None:
            return {"success": False, "error": "missing_limit_price", "detail": "Limit orders require limit_price"}
        ...
        alpaca_side = side_map.get(side.lower())
        if not alpaca_side:
            return {"success": False, "error": "invalid_side", "detail": f"Invalid side: {side}"}
        ...
        order_data = {
            ...
            "type": order_type,
            "time_in_force": time_in_force,
            "order_class": "simple"  # Options orders are simple (not bracket/OCO)
        }
        
        if limit_price is not None:
            order_data["limit_price"] = limit_price
        
        logger.info(f"📤 Placing {side} option order: {qty} {occ_symbol} @ {order_type}")
        
        response = self._make_request("POST", "/v2/orders", json_data=order_data)
        
        if response is None:
            logger.error(f"❌ Options order submission failed for {occ_symbol}")
            return {
                "success": False,
                "symbol": occ_symbol,
                "error": "request_failed",
        ...
        if "error" in response or "code" in response:
            ...
            logger.error(f"❌ Options order rejected: {error_msg}")
            return {
                "success": False,
                "symbol": occ_symbol,
                "error": response.get("code", "api_error"),
                "detail": error_msg
            }
        ...
        logger.info(f"✅ Options order placed: {order_id} (status: {status})")
