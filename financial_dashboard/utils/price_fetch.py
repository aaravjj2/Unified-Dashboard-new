...
# ENV HOTFIX: Always load environment and validate keys (graceful fallback)
try:
    from .load_env import load_environment
    # Load environment but don't fail if keys are missing - just log warning
    env_status = load_environment(raise_on_missing=False)
    if not env_status.get('valid', False):
        import logging
        _logger = logging.getLogger(__name__)
        _logger.warning(f"[price_fetch] ENV loaded but missing keys: {env_status.get('missing_keys', [])}")
    else:
        print(f"[price_fetch] ENV loaded: valid={env_status.get('valid')}")
except ImportError as exc:
    import logging
    _logger = logging.getLogger(__name__)
    _logger.warning(f"[price_fetch] ENV loader not available: {exc}")
except Exception as exc:
    import logging
    _logger = logging.getLogger(__name__)
    _logger.warning(f"[price_fetch] ENV load warning: {exc}")
