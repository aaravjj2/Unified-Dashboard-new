...
def _safe_float(val) -> float:
    """Convert a value or array-like to a single float safely.

    - If `val` is already scalar convert directly.
    - If `val` is array-like, return the first element as float (fallback).
...
def save_shap_explanations(
    ...
    for i, ticker in enumerate(tickers):
        ...
        # Fix for TypeError: only length-1 arrays can be converted to Python scalars
        feature_importance = []
        for name, val in zip(feature_names, ticker_shap):
            try:
                val_float = float(val)
            except TypeError:
                # If val is an array (e.g. for classification), take the first element or mean
                import numpy as np
                if isinstance(val, (list, tuple, np.ndarray)):
                    val_float = float(val[0]) if len(val) > 0 else 0.0
                else:
                    val_float = 0.0
            feature_importance.append((name, val_float))
        ...
        shap_sum = _safe_float(np.sum(np.asarray(ticker_shap)))
        ...
        explanations[ticker] = {
            ...
            'all_features': [
                {'feature': name, 'shap_value': _safe_float(val)}
                for name, val in zip(feature_names, ticker_shap)
            ]
        }
